...
def make_packet(header,data):
	...
	# x.addline('header:'+header)
	# header_str = binascii.unhexlify('%x'%header)
	data = header + data
	return data
...
while going:
	...
	if r == FD_READY:
		...
		data,num_blocks,chinese = txt2bit(line)
		binary_header,encapsulate_data = reliability.encapsulate(data)
		x.addline('header:'+binary_header)
		p = struct.pack('B',int(binary_header,2))
		x.addline(p)
		send_data = make_packet(p,line)
		n1.send(n,send_data)
		...
		# X.addline('recived header:'+str(up))
#x.stop()
